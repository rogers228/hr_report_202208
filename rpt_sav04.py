...
class Report_sav04(tool_excel):
    ...
    def output(self):
        ...
        for caid in lis_ca:
            ...
            self.c_write(cr+1, 1,  f'公司: {self.hr.ca01Getca03(caid)}', font_A_10) # 訂餐備註
            self.c_write(cr+2, 1,  f'薪資年月: {self.YM}', font_A_10) # 訂餐備註
            lis_col = ['人員','姓名','轉帳(一)','轉帳(二)','轉帳(三)','實領薪資']
            for i, c in enumerate(lis_col):
                self.c_write(cr+3, i+1, c, font_A_10, border=bt_border)
            cr += 4

            df_w = df_rs.loc[(df_rs['ps40'] == caid)] # 篩選 (以公司為群組)
